...
"""
Sistema de Gestión de Operaciones de Venta BRL-ARS
Automatiza el proceso de generación de presupuestos para venta de BRL contra ARS
"""
...
# Constantes
CARPETA_OPERACIONES = "Operaciones de venta"
...
def calcular_operacion(cotizacion_brl_usd, cotizacion_usd_ars, importe_brl, ganancia_brl, ganancia_ars):
    ...
    Args:
        ...
        importe_brl (float): Importe a vender en BRL
...
- *Total a abonar en pesos argentinos*: {formatear_numero_mensaje(total_ars)} ARS
...
def guardar_operacion(datos_operacion, carpeta_operaciones):
    ...
    nombre_archivo = f"{timestamp}_{datos_operacion['apellido_cliente'].upper()}_{datos_operacion['nombre_cliente'].upper()}_venta_{int(datos_operacion['importe_brl'])}BRL.json"
...
def main():
    ...
    print("\n" + "="*50)
    print("SISTEMA DE GESTIÓN DE VENTA BRL/ARS".center(50))
    print("="*50 + "\n")
    ...
    importe_brl = float(input("Ingrese el importe en BRL que desea vender: "))
    ...
    linea_ticket = f"{codigo_ticket} - {fecha_actual} - {apellido_cliente} {nombre_cliente} - VENTA x BRL {formatear_numero_mensaje(importe_brl)}."
...
ENTREGAMOS: {formatear_numero_mensaje(importe_brl)} BRL
RECIBIMOS: {formatear_numero_mensaje(resultados['total_ars'])} ARS
